...
// Hizmet kartı gridi — anasayfa bölümü ve /hizmetler liste sayfası ortak kullanır (DRY).
export function ServicesGrid() {
  const t = useTranslations("services");
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {services.map(({ key, slug, Icon }, i) => (
        <Reveal key={key} delay={i * 0.08}>
          ...
          >
            ...
            <div className="flex flex-1 flex-col p-8">
              <h3 className="font-display text-h4 font-medium">{t(`${key}.title`)}</h3>
              <p className="mt-3 flex-1 text-body text-text-muted">{t(`${key}.desc`)}</p>
